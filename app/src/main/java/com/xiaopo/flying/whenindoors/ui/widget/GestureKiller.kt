...
class GestureKiller(val context: Context) {
  ...
  private val lastMidPoint = PointF()
        ...
        if (gesture == GestureType.DRAG) {
          ...
          if (event.pointerCount >= 2) {
            ...
            onZoomAndRotateListener
                ?.invoke(midPoint.x, midPoint.y,
                    currentDistance / lastDistance, currentRotation - lastAngle,
                    midPoint.x - lastMidPoint.x, midPoint.y - lastMidPoint.y)

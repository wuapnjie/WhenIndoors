 ...
 * @author wupanjie
 */
class RoomItemViewBinder : AnotherBinder<Room>() {
  ...
  override fun renderView(holder: AnotherViewHolder, itemView: View, item: Room) {
    itemView.tv_room_name.text = item.name
    itemView.tv_fingerprint_count.text = "${item.positions.size ?: 0} 个位置指纹"

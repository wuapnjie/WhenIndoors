...
infix fun <V : Any, E : Throwable> Result<V, E>.getOrElse(fallback: V) = when (this) {
  is Result.Success -> value
  else -> fallback
}

fun <V : Any, U : Any, E : Throwable> Result<V, E>.map(transform: (V) -> U): Result<U, E> = when (this) {
  is Result.Success -> Result.Success<U, E>(transform(value))
  is Result.Failure -> Result.Failure<U, E>(error)
}

fun <V : Any, U : Any, E : Throwable> Result<V, E>.flatMap(transform: (V) -> Result<U, E>): Result<U, E> = when (this) {
...
sealed class Result<out V : Any, out E : Throwable> {
  ...
  class Success<out V : Any, out E : Throwable>(val value: V) : Result<V, E>() {
  ...
  class Failure<out V : Any, out E : Throwable>(val error: E) : Result<V, E>() {
